...
function SubBrandTable({ subBrands, brands, onEdit, onDelete }) {
  ...
  const filteredSubBrands = useMemo(() => {
    ...
    return subBrands.filter(subBrand =>
      subBrand.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      getBrandName(subBrand.brandId).toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        ...
        <p className="text-sm text-gray-600">
          <span className="font-semibold text-blue-600">{paginatedSubBrands.length}</span> dari <span className="font-semibold text-blue-600">{filteredSubBrands.length}</span> sub brands ditampilkan
        </p>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-blue-600 text-white">
              ...
              <th className="px-6 py-3 text-left text-sm font-bold flex-1">Nama Sub Brand</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {paginatedSubBrands && paginatedSubBrands.length > 0 ? (
              paginatedSubBrands.map((subBrand, index) => (
                <tr 
                  key={subBrand.id} 
              ...
              <tr>
                <td colSpan="4" className="px-6 py-12 text-center">
                  <p className="text-gray-500 text-sm">
                    {filteredSubBrands.length === 0 && searchTerm 
                      ? "❌ Tidak ada sub brand yang sesuai" 
                      : "🏷️ Tidak ada sub brand"}
                  </p>
                </td>
              </tr>
            )}
