...
const parseWeekTotal = (data) =>
  WEEKS.map(w => parseInt(String(data?.[w] || "0").replace(/\./g, "")) || 0)
       .reduce((a, b) => a + b, 0)
       .toLocaleString("id-ID");

export default function ForecastCard({ forecastData, editForecastData, onSave, onChange }) {
    ...
    <>
      <div className="flex items-center justify-between mb-6 p-4 rounded-xl" style={{ background: "linear-gradient(135deg, #eff6ff, #dbeafe)", border: "1px solid #bfdbfe" }}>
        ...
        <div className="text-right">
          ...
          <p className="text-base font-bold" style={{ color: "#2563eb" }}>Rp {parseWeekTotal(data)}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 flex-1">
        {WEEKS.map((w, i) => {
          ...
          return (
            <div key={w} className="p-4 rounded-xl flex flex-col justify-between" style={{ background: bg, border: `1px solid ${border}` }}>
              <div className="flex items-center gap-2 mb-2">
                <div className="w-2 h-2 rounded-full" style={{ background: color }} />
                <p className="text-xs font-semibold uppercase tracking-wider" style={{ color }}>Week {i + 1}</p>
              </div>
              <p className="text-sm font-bold" style={{ color: "#1e293b" }}>Rp {data[w]}</p>
            </div>
          );
        })}
      </div>
...
function EditForm({ data, onChange, onSave, onCancel }) {
  const fields = [{ key: "plan", label: "Plan" }, ...WEEKS.map((w, i) => ({ key: w, label: `Week ${i + 1}` }))];
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-2 mt-1">
        <button onClick={onSave} className="flex-1 flex items-center justify-center gap-2 text-white text-sm font-semibold py-2 rounded-lg" style={{ background: "#2563eb" }}>
          <Save size={14} /> Save
        </button>
        <button onClick={onCancel} className="flex-1 flex items-center justify-center gap-2 text-sm font-semibold py-2 rounded-lg" style={{ background: "#f1f5f9", color: "#64748b" }}>
          <X size={14} /> Cancel
        </button>
      </div>
    </div>
