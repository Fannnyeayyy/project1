...
export default function StatCards({ loading, totalSales, totalProducts, totalSubBrands, fmtM }) {
  const cards = [
    { label: "Total Sales",     value: loading ? "—" : fmtM(totalSales), color: "#2563eb", bg: "#eff6ff", Icon: DollarSign },
    { label: "Total Product",   value: loading ? "—" : totalProducts,    color: "#10b981", bg: "#d1fae5", Icon: Package },
    { label: "Total Sub Brand", value: loading ? "—" : totalSubBrands,   color: "#7c3aed", bg: "#ede9fe", Icon: Tag },
  ];
      ...
      {cards.map(({ label, value, color, bg, Icon }) => (
        <div key={label} className="bg-white rounded-xl p-6 flex items-start justify-between" style={{ border: "1px solid #e2e8f0" }}>
          <div>
            ...
            <p className="text-3xl font-bold tracking-tight" style={{ color: "#1e293b" }}>{value}</p>
