/**
 * FormStockIndomaret.jsx
 * Total Value dihitung otomatis: avgL3m × hargaPerCarton dari product.
...
function FormStockIndomaret({ isOpen, onClose, onSubmit, editData, brands = [], subBrands = [], products = [] }) {
  const empty = { brandId: "", subBrandId: "", productId: "", avgL3m: "", isActive: true };
  ...
  // Preview total value = avgL3m × hargaPerCarton
  const previewTotal = (parseInt(form.avgL3m) || 0) * hargaPerCarton;
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.brandId || !form.subBrandId || !form.productId || !form.avgL3m) return alert("Semua field wajib diisi");
    setLoading(true);
    await onSubmit({
      ...form,
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: "rgba(0,0,0,0.5)" }}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md" style={{ border: "1px solid #e2e8f0" }}>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div>
            <label style={labelStyle}>Avg L3M (Karton)</label>
          ...
          <div>
            ...
            <div className="px-3 py-2 rounded-lg text-sm font-bold" style={{ background: "#d1fae5", border: "1px solid #a7f3d0", color: "#10b981" }}>
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" name="isActive" checked={form.isActive} onChange={handleChange} id="active-si" className="w-4 h-4" style={{ accentColor: "#10b981" }} />
            <label htmlFor="active-si" className="text-sm font-medium" style={{ color: "#64748b" }}>Aktif</label>
          </div>
